...
# Files in listings/ generated by a different script (scrape_india.py) that
# this script does not own and must never delete during cleanup.
EXTERNALLY_MANAGED_LISTING_FILES = {"domestic-india.md"}
...
def write_listings(listings, fetched_sources):
    ...
    for stale in existing_files - written_files:
        if stale.name in EXTERNALLY_MANAGED_LISTING_FILES:
            continue
        stale.unlink()
    ...
    index.append(
        "\n---\n"
        "**[Domestic India roles](listings/domestic-india.md)** -- a separate, "
        "auto-refreshed list of India-based postings pulled directly from a "
        "few big tech companies' own career-site APIs, every ~3 days. List "
        "only -- not part of the auto-refresh above and not integrated with "
